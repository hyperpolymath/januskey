...
____
*Source of truth:* `+.machine_readable/descriptiles/STATE.a2ml+` (completion 60%,
...
they win. Percentages below are qualitative, not measured coverage. This
agreement is now *machine-enforced*: `+just check-dashboard+` (crate
`+crates/dashboard-check+`, CI job `+Dashboard Check+`) fails the build
