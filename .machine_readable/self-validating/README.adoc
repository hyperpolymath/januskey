...
[horizontal]
`januskey-metadata.k9.ncl`:: Kennel-level guard carrying januskey's security
invariants (banned/required crypto algorithms, safety invariants for crypto
code, test thresholds, language policy, RSR compliance flags).
`methodology-guard.k9.ncl`:: Validator that checks agent work respects
`bot_directives/methodology.a2ml` (believe_me / assert_total ceilings —
januskey's Idris2 ABI proofs in `src/abi/` hold both at 0 — plus STATE
placeholder rejection and coverage staleness).

Repo-real validation entry points the components point at: `just build`
(cargo build --workspace --release), `just test` (cargo test --workspace),
`just test-contracts`, `just test-proofs` (Idris2 ABI check).
